...
export default function GamesPage() {
    ...
    const [newGames, setNewGames] = useState<AppItem[]>([]);
    ...
    useEffect(() => {
        Promise.all([
            ...
            .then(([categoryData, topNewData]: [{ app: Category[], game: Category[] }, TopNewData]) => {
                setCategories(categoryData.game);
                setTopDownloads(topNewData.top);
                setNewGames(topNewData.new);
                setIsLoading(false);
            })
            .catch(err => {
            ...
            <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                {isLoading ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        {/* Game Categories - Takes 2 columns on large screens */}
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {categories.map((category) => (
                                        <a
                                            ...
                                            className="flex items-center p-3 rounded-lg border border-gray-200 hover:border-purple-500 hover:bg-purple-50 transition-all group"
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <h3 className="text-sm font-medium text-gray-900 group-hover:text-purple-600 truncate">
                                                    {category.title}
                                                </h3>
                                            </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                    {newGames.map((game, index) => (
                                        <a
                                            ...
                                            href={`${game.link}.html`}
                                            rel="noopener noreferrer"
                                            className="flex flex-col p-4 rounded-lg border border-gray-200 hover:border-orange-500 hover:bg-orange-50 transition-all group"
                                        >
                                            <div className="w-16 h-16 mx-auto mb-3 relative bg-gray-100 rounded-lg overflow-hidden">
                                                <Image
                                                    src={`${process.env.NEXT_PUBLIC_OSS_URL}/${game.icon}`}
                                                    alt={game.name}
                                                    fill
                                            ...
                                            <h3 className="text-sm font-medium text-gray-900 text-center truncate group-hover:text-orange-600 mb-2">
                                                {game.name}
                                            </h3>
